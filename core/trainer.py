...
from core.loss import AdversarialLoss, FocalFrequencyLoss, LPIPSLoss, PerceptualLoss
...
class Trainer:
    def __init__(self, config):
        ...
        # ── Focal Frequency Loss — instantiated only if weight > 0 ───────────
        if self.config["losses"].get("ffl_weight", 0) > 0:
            self.ffl_loss = FocalFrequencyLoss(
                loss_weight=self.config["losses"]["ffl_weight"],
                alpha=self.config["losses"].get("ffl_alpha", 1.0),
            ).to(self.config["device"])
        # ─────────────────────────────────────────────────────────────────────
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            # ── focal frequency loss — hole region only ───────────────────────
            if self.config["losses"].get("ffl_weight", 0) > 0:
                # Apply ONLY to the masked/hole region (same as hole_loss).
                # Unmasked regions are already perfect copies of the input;
                # including them would pollute the gradient signal.
                pred_hole = (pred_imgs * masks).view(-1, 3, h, w)
                gt_hole = (frames * masks).view(-1, 3, h, w)
                ffl = self.ffl_loss(pred_hole, gt_hole)
                gen_loss += ffl
                self.add_summary(self.gen_writer, "loss/ffl_loss", ffl.item())
            # ──────────────────────────────────────────────────────────────────
            # gan loss
            if not self.config["model"]["no_dis"]:
